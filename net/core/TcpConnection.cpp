...
void TcpConnection::send(const std::string &buf)
{
    ...
    {
        loop_->runInLoop([self = shared_from_this(), buf]()
...
void TcpConnection::sendInLoop(const void *data, size_t len)
{
    ...
    // 表示channel_第一次开始写数据，且缓冲区没有待发送数据
    if (!channel_->isWriting() && outputBuffer_.readableBytes() == 0)
    {
        ...
        if (nwrote >= 0)
        {
            ...
            {
                // 既然在这里一次性把数据发送完成，就不用再给channel设置epollout事件
                loop_->queueInLoop([self = shared_from_this()]()
                                   { self->writeCompleteCallback_(self); });
            }
        }
    ...
    // 说明当前这一次write并没有将数据全部发送，所以剩余的数据需要保存到缓冲区中
    // 然后给Channel注册epollout事件，poller发现tcp的发送缓冲区有空间，会通知相应的sock->channel,调用writeCallback回调
    // 最终也就是调用TcpConnection::handleWrite函数，把发送缓冲区中的数据全部发送完成
    if (!faultError && remaining > 0)
    {
        ...
        {
            loop_->queueInLoop([self = shared_from_this(), oldLen, remaining]()
